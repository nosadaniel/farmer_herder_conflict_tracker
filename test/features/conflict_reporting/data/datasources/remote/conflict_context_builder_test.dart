...
void main() {
  ...
  group('ConflictContextBuilder.build with wizardAnswers', () {
    ...
    test('omits missing parts when only whatsHappening is present', () async {
      ...
      expect(block, contains('Report: "Herd moving toward farmland."'));
      expect(block, isNot(contains('People involved:')));
      expect(block, isNot(contains('Additional detail:')));
    });

    test(
      'falls back to a neutral string for an empty wizardAnswers map',
      () async {
        final block = await builder.build(
          lat: 9.0,
          lng: 8.5,
        ...
        expect(
          block,
          contains('Report: "Report submitted via guided wizard."'),
        );
      },
    );

    test(
      'does not break quoting when detailText has special characters',
      () async {
        final block = await builder.build(
          lat: 9.0,
          lng: 8.5,
  ...
  group('ConflictContextBuilder.build exactly-one-of assertion', () {
    test(
      'providing none of reportText/actionEventName/wizardAnswers asserts',
      () {
        expect(
          () => builder.build(
            lat: 9.0,
  ...
  group(
    'ConflictContextBuilder.build existing reportText/actionEventName paths',
    () {
