...
4. **`.github/workflows/_build_test.yml`** (reusable workflow, `on: workflow_call`) — the single source of truth for "does this pass." Restores the Firebase config, sets up Flutter, regenerates Drift/Riverpod codegen (`dart run build_runner build --delete-conflicting-outputs`), then `flutter analyze` → `dart format --output=none --set-exit-if-changed .` → `flutter test`. Declares the three Firebase secrets it needs via `on.workflow_call.secrets`; callers pass `secrets: inherit` rather than re-listing each one. Any workflow can gate on this by calling it as a job via `uses: ./.github/workflows/_build_test.yml` with `secrets: inherit`, instead of re-declaring analyze/format/test steps. **This is the only job that regenerates codegen** — the `.g.dart` files are committed to git (not gitignored) and both deploy jobs trust them as-is, since they `needs: gate` and this job already validated they're fresh. If a track edits a `@riverpod` provider or Drift table, committing the regenerated `.g.dart` alongside it is on that track, not on CI. **No longer builds a debug APK** (see "Planned CI/CD redesign," now implemented below) — that was redundant with the real release build each deploy job does right after this gate passes; dropping it gives every push a faster analyze/format/test-only signal.
...
| `deploy-android.yml` | **push to `main`** (implemented from the "Planned CI/CD redesign" below) + `workflow_dispatch` as a manual fallback | Job 1: calls `_build_test.yml` as a gate (`secrets: inherit`). Job 2 (needs job 1, `environment: android-production`): `restore-firebase-config` → `restore-android-signing` → `setup-flutter` → `flutter build apk --release` (real signed build) → upload to Firebase App Distribution via `wu-vincent/firebase-app-distribution-github-action`. |
| `deploy-web.yml` | **push to `main`** + `workflow_dispatch` fallback, **only meaningful if web isn't cut** | Same pattern: gate on `_build_test.yml`, then `restore-firebase-config` → `setup-flutter` → `flutter build web --release` → publish `build/web` to the `gh-pages` branch (`peaceiris/actions-gh-pages@v4`). `deploy` job now runs under `environment: web-production`. |
...
**Deploy jobs were manual-trigger-only through Phase 1** — during the parallel-tracks fan-out the repo got many WIP pushes; auto-deploying on every push would have either spammed Firebase App Distribution testers or failed loudly on incomplete branches. Now that Phase 1–5 are merged to `main` and pushes there are deliberate, both deploy workflows trigger on `push: branches: [main]` (see "Planned CI/CD redesign," now implemented) — `workflow_dispatch` stays available as a manual fallback.
...
## CI/CD redesign — reviewed deployments — **implemented 2026-09-21**

Original state (`ci.yml` built a debug APK on every push; deploy workflows were `workflow_dispatch`-only) held through Phase 1's fan-out as planned, then switched over once Phase 1–5 landed on `main`:

1. **`ci.yml`/`_build_test.yml` is now lint + format + test only** — the `flutter build apk --debug` step is gone from what runs on every push. Steps: `restore-firebase-config` → `setup-flutter` → codegen → `flutter analyze` → `dart format --output=none --set-exit-if-changed .` → `flutter test`. ✅ Verified locally before landing: `flutter analyze` clean, `dart format` clean (the 20 files it initially flagged — mostly the already-merged wizard feature — have been reformatted), all 58 tests pass.
2. **`deploy-android.yml` / `deploy-web.yml` now trigger on `push: branches: [main]`**, `workflow_dispatch` kept as a manual fallback. Their `deploy` jobs run under `environment: android-production` / `environment: web-production` respectively (split, not shared, so each can be approved independently).
...
**⚠️ Not yet done — blocks the "reviewed deployment" part of this from actually enforcing anything**: `android-production`/`web-production` don't exist as GitHub Environments yet. Until you create them in Settings → Environments and add required reviewers, referencing `environment: ...` in these workflows is a no-op — GitHub auto-creates an unprotected environment on first use, so **pushes to `main` will auto-deploy immediately, with no pause**, exactly like before. That's actually the right behavior for today's submission deadline (no extra manual step needed to ship). Add the required-reviewers protection rule *after* today's submission is safely in, not before — see "Your responsibilities" #12.
