...
|---|---|---|---|---|
| **A — Map & Historical Data** | 🔄 running | `lib/features/map/**`, `lib/features/conflict_reporting/data/{datasources,models}` for conflict data | Load & parse `farmer_herder_conflict.json` into Drift `ConflictData`, `flutter_map` + `latlong2` rendering with hotspot markers, GPS via `geolocator` (with permission-denied fallback to manual/last-known location) | Phase 0 |
